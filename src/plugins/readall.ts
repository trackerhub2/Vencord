...
module.exports = ((_) => {
  ...
  return !window.BDFDB_Global ||
    ...
    : (([Plugin, BDFDB]) => {
        ...
        const ReadAllButtonComponent = class ReadAllButton extends BdApi.React
          .Component {
          clearClick() {
            if (_this.settings.batch.guilds)
              this.clearGuilds(
                _this.settings.batch.muted ? this.getGuilds() : this.getUnread()
              );
          ...
          getGuilds() {
            return BDFDB.LibraryModules.SortedGuildUtils.getFlattenedGuilds()
              .map((g) => g.id)
              .filter((n) => n);
          }
          getUnread() {
          ...
          getSettingsPanel(collapseStates = {}) {
            ...
            let listInstance = null,
              batchSetGuilds = (value) => {
                if (!value) {
                  for (let id of BDFDB.LibraryModules.SortedGuildUtils.getFlattenedGuildIds())
                    blacklist.push(id);
